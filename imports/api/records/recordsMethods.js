  ...
  reportManagement: async function ({
    ...
    const pages = await recordsCollection
      ...
      .aggregate([
        ...
        },
        {
          $facet: {
            ...
            data: [
              ...
              {
                $project: {
                  // Condicional basado en el valor de "project"
                  allFields: {
                    $cond: {
                      if: { $eq: ["$project", "sierra"] },
                      then: "$$ROOT", // Retorna todo el documento si el project es "sierra"
                      else: {
                        PRODUCTO: 1,
                        CONTRATO: 1,
                        CLIENTE: 1,
        ...
        {
          $project: {
            data: "$data.allFields", // Mantener solo los datos y el conteo total en la salida
            totalCount: { $arrayElemAt: ["$metadata.totalCount", 0] },
          },
        },
      ])
